...
func (m *openstackMachineAndInfra) ToMachineAndInfrastructureMachine() (*capiv1.Machine, client.Object, []string, error) {
	capiMachine, capoMachine, warnings, errs := m.toMachineAndInfrastructureMachine()
...
func (m *openstackMachineAndInfra) toMachineAndInfrastructureMachine() (*capiv1.Machine, client.Object, []string, field.ErrorList) {
	...
	capoMachine, warn, machineErrs := m.toOpenStackMachine(openstackProviderConfig)
	if machineErrs != nil {
	...
	return capiMachine, capoMachine, warnings, errs
}
...
func (m *openstackMachineSetAndInfra) ToMachineSetAndMachineTemplate() (*capiv1.MachineSet, client.Object, []string, error) { //nolint:dupl
	...
	capiMachine, capoMachineObj, warn, err := m.toMachineAndInfrastructureMachine()
	if err != nil {
...
func (m *openstackMachineAndInfra) toOpenStackMachine(providerSpec mapiv1alpha1.OpenstackProviderSpec) (*capov1.OpenStackMachine, []string, field.ErrorList) {
	...
	ports, warn, err := convertMAPOPortsToCAPO(fldPath.Child("ports"), providerSpec.Ports)
	if err != nil {
	...
	networkPorts, warn, err := convertMAPONetworksToCAPO(fldPath.Child("networks"), providerSpec.Networks)
	if err != nil {
		errs = append(errs, err...)
	}
	...
	additionalBlockDevices, warn, err := convertMAPOAdditionalBlockDevicesToCAPO(fldPath.Child("additionalBlockDevices"), providerSpec.AdditionalBlockDevices)
	if err != nil {
		errs = append(errs, err...)
	}
	...
	rootVolume, warn, err := convertMAPORootVolumeToCAPO(fldPath.Child("rootVolume"), providerSpec.RootVolume)
	if err != nil {
		errs = append(errs, err...)
	}
...
func convertMAPOAdditionalBlockDevicesToCAPO(fldPath *field.Path, mapoAdditionalBlockDevices []mapiv1alpha1.AdditionalBlockDevice) (*[]capov1.AdditionalBlockDevice, []string, field.ErrorList) {
	errs := field.ErrorList{}
	warnings := []string{}
	...
	for i, mapoAdditionalBlockDevice := range mapoAdditionalBlockDevices {
		...
		if mapoAdditionalBlockDevice.Storage.Type == mapiv1alpha1.VolumeBlockDevice {
			if mapoAdditionalBlockDevice.Storage.Volume == nil {
				...
				errs = append(errs, field.Required(fldPath.Index(i).Child("volume"), "volume is required, but is missing"))
				continue
			}
	...
	return &capoAdditionalBlockDevices, warnings, errs
}
...
func convertMAPONetworksToCAPO(fldPath *field.Path, mapoNetworks []mapiv1alpha1.NetworkParam) ([]capov1.PortOpts, []string, field.ErrorList) { //nolint:gocognit,cyclop
	...
	for i, mapoNetwork := range mapoNetworks {
		...
		if networkID == "" {
			networkID = mapoNetwork.Filter.ID //nolint:staticcheck
		}
		...
		if networkID == "" && (mapoNetwork.Filter == mapiv1alpha1.Filter{}) { //nolint:nestif
			...
			for j, mapoSubnet := range mapoNetwork.Subnets {
				...
				if mapoSubnet.Filter.NetworkID != "" { //nolint:staticcheck
					// Field exists in the API but is never used within the codebase.
					warnings = append(warnings, field.Invalid(fldPath.Index(i).Child("subnets").Index(j).Child("filter", "networkID"), mapoSubnet.Filter.NetworkID, "fixedIp is ignored by MAPO, ignoring").Error()) //nolint:staticcheck
				}
	...
	return capoPorts, warnings, errs
}
...
func convertMAPORootVolumeToCAPO(fldPath *field.Path, mapoRootVolume *mapiv1alpha1.RootVolume) (*capov1.RootVolume, []string, field.ErrorList) {
	...
	if mapoRootVolume.DeprecatedDeviceType != "" { //nolint:staticcheck
		// deviceType is deprecated and silently ignored.
		// See
		warnings = append(warnings, field.Invalid(fldPath.Child("deviceType"), mapoRootVolume.DeprecatedDeviceType, "deviceType is silently ignored by MAPO and will not be converted").Error()) //nolint:staticcheck
	}

	if mapoRootVolume.DeprecatedSourceType != "" { //nolint:staticcheck
		// sourceType is deprecated and silently ignored.
		// See
		warnings = append(warnings, field.Invalid(fldPath.Child("sourceType"), mapoRootVolume.DeprecatedSourceType, "sourceType is silently ignored by MAPO and will not be converted").Error()) //nolint:staticcheck
	}
	...
	return capoRootVolume, warnings, errs
}
